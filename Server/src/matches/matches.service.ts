...
export class MatchesService {
  ...
  async getMatches(date?: Date): Promise<any> {
    ...
    try {
      ...
      matches = matches.filter(
        (match) =>
          new Date(match.startTimestamp * 1000).getDate() == date.getDate(),
      );
      return matches;
    } catch (error) {
      console.log(error);
  ...
  async getUserMatches(userId: number) {
    ...
    let favMatches = await Promise.all(
      favTeams.map(async (team) => {
        let nextMatches = await this.getTeamMatches(team.getId()).then(
          (res) => res.next,
        );
    ...
    let notFavMatches = allMatches.filter(
      (match) =>
        !favTeamIds.includes(match.homeTeam.id) &&
        !favTeamIds.includes(match.awayTeam.id),
    );
  ...
  async getCompMatches(
    ...
    try {
      ...
      return matches?.slice(
        prev ? matches.length - limitSet : 0,
        prev ? matches.length : limitSet,
      );
    } catch (e) {
      console.log(e.message);
      return [];
