...
const TeamDisplay: React.FC<TeamDisplayProps> = ({ team, isWinner, isNationalTeam }) => {
  const content = (
...
const KnockoutBracket: React.FC<KnockoutBracketProps> = ({ matches, isNationalTeam = false }) => {
  ...
  const matchesByStage = knockoutMatches.reduce<Record<string, Match[]>>((acc, match) => {
  ...
  const getMatchResult = (match: Match): MatchResult | null => {
