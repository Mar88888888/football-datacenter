...
import type { Competition, Match, Standings, TableEntry } from '../types';

interface TeamCellProps {
  team: TableEntry['team'] | null;
  isNationalTeam: boolean;
}

const TeamCell: React.FC<TeamCellProps> = ({ team, isNationalTeam }) => {
...
const GroupPage: React.FC = () => {
  const { competitionId, groupName } = useParams<{ competitionId: string; groupName: string }>();
  ...
  const {
    ...
    loading: loadingComp,
    error: compError,
  } = useApi<Competition>(API_ENDPOINTS.COMPETITION(competitionId!));

  const {
    data: standings,
    loading: loadingStandings,
    error: standingsError,
  } = useApi<Standings>(API_ENDPOINTS.STANDINGS(competitionId!));

  const {
    data: matchesData,
    loading: loadingMatches,
    error: matchesError,
  } = useApi<Match[]>(API_ENDPOINTS.COMPETITION_MATCHES(competitionId!));
  ...
  const groupTeamIds = useMemo(() => {
    if (!groupStandings?.table) return new Set<number>();
